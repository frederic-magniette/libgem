...
struct circle *iRCP_old(struct dataset *ds,struct weights *w) {
  ...
  while(1) {
    ...
    nb_iter++;
    if (nb_iter>200) { //try smaller amplitude to make algorithm converge
      amplitude=0.2;
    }
    ...
    for(p=0;p<ds->nb_points;p++) {
      unimove=vector_by2points(ds->points[p],result->center);
      normalize_vector(unimove);
      lambda=amplitude*((mean_distrib(dd)-dists[p])/mean_distrib(dd))*w->coefs[p];
      if (!isnan(lambda) && !isnan_vector(unimove)) {
        mult_lambda_vect(unimove,lambda);
        add_vector(move,unimove);
      } 
